...
# This is for Deflection version 2.x. The main changes from version 1.x are:
# - change the way pieces can move: one space instead of unlimited.
# - a piece can either move or rotate in a turn, but not both.
#
# These changes are intended to make it easier to implement an AI for the game
# since there are fewer possible moves to consider at each step
...
class Board:
    ...
    def is_legal_move(self, src: Tuple[int, int], tgt: Tuple[int, int]) -> bool:
        ...
        if mag > 1:
            return False
        # check that target position is on the core board
        if tgt[0] < 1 or tgt[0] > 8:
            return False
        if tgt[1] < 1 or tgt[1] > 8:
            return False
        # check that there is no piece at the current position
        s, _ = self.get_piece(tgt[0], tgt[1])
        if s != Piece.EMPTY:
            return False
        return True
